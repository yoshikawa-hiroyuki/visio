...
//----------------------------------------------------------------
BEGIN_EVENT_TABLE(vsnMPP_Scatter_plotArrows, wxPanel)
  EVT_TEXT_ENTER(MPP_Scatter_plotArrows_VecScaleTxt,
                 vsnMPP_Scatter_plotArrows::OnVecScaleTxt)
  EVT_CHECKBOX(MPP_Scatter_plotArrows_VecHeadChk,
               vsnMPP_Scatter_plotArrows::OnVecHeadChk)
  EVT_CHECKLISTBOX(MPP_Scatter_plotArrows_VecDataChkLst,
                   vsnMPP_Scatter_plotArrows::OnVecDataChkLst)
  EVT_COMBOBOX(MPP_Scatter_plotArrows_SelDataLst,
               vsnMPP_Scatter_plotArrows::OnSelDataLst)
  EVT_CHECKBOX(MPP_Scatter_plotArrows_UpdMinMaxChk,
               vsnMPP_Scatter_plotArrows::OnUpdMinMaxChk)
  EVT_TEXT_ENTER(MPP_Scatter_plotArrows_LineWidthTxt,
                 vsnMPP_Scatter_plotArrows::OnLineWidthTxt)
  EVT_CHECKBOX(MPP_Scatter_plotArrows_AntiAliasChk,
               vsnMPP_Scatter_plotArrows::OnAntiAliasChk)
END_EVENT_TABLE()


/* constructors / destructor */

vsnMPP_Scatter_plotArrows::vsnMPP_Scatter_plotArrows(wxPanel* parent,
						     vsnMethodObj* pm)
  : vsnMethodPP(parent, pm)
{
  assert(parent);
  assert(dynamic_cast<vsnMethod_Scatter_plotArrows*>(pm));

  // create widgets
  m_pVecScaleTxt = new wxTextCtrl(this, MPP_Scatter_plotArrows_VecScaleTxt,
                                  wxT(""), wxDefaultPosition, wxDefaultSize,
                                  wxTE_PROCESS_ENTER);
  assert(m_pVecScaleTxt);
  m_pVecHeadChk = new wxCheckBox(this, MPP_Scatter_plotArrows_VecHeadChk,
                                 wxT("arrow head"));
  assert(m_pVecHeadChk);
  m_pVecDataChkLst = new wxCheckListBox(this,
                                        MPP_Scatter_plotArrows_VecDataChkLst);
  assert(m_pVecDataChkLst);
  m_pSelDataLst = new wxComboBox(this, MPP_Scatter_plotArrows_SelDataLst,
                                 wxT(""), wxDefaultPosition, wxDefaultSize,
                                 0, NULL, wxCB_READONLY);
  assert(m_pSelDataLst);
  m_pUpdMinMaxChk = new wxCheckBox(this, MPP_Scatter_plotArrows_UpdMinMaxChk,
                                   wxT("update lut minmax"));
  assert(m_pUpdMinMaxChk);
  m_pLineWidthTxt = new wxTextCtrl(this, MPP_Scatter_plotArrows_LineWidthTxt,
                                   wxT(""), wxDefaultPosition, wxDefaultSize,
                                   wxTE_PROCESS_ENTER);
  assert(m_pLineWidthTxt);
  m_pAntiAliasChk = new wxCheckBox(this, MPP_Scatter_plotArrows_AntiAliasChk,
                                   wxT("anti-alias line"));
...
vsnMPP_Scatter_plotArrows::~vsnMPP_Scatter_plotArrows() {
}


/* interface */

bool vsnMPP_Scatter_plotArrows::update() {
  register size_t i;
  char txt[64];

  vsnMethod_Scatter_plotArrows* pm
    = dynamic_cast<vsnMethod_Scatter_plotArrows*>(p_method);
  if ( ! pm ) return false;
...
void vsnMPP_Scatter_plotArrows::OnVecScaleTxt(wxCommandEvent& event) {
...
void vsnMPP_Scatter_plotArrows::OnVecHeadChk(wxCommandEvent& event) {
...
void vsnMPP_Scatter_plotArrows::OnVecDataChkLst(wxCommandEvent& event) {
...
void vsnMPP_Scatter_plotArrows::OnSelDataLst(wxCommandEvent& event) {
...
void vsnMPP_Scatter_plotArrows::OnUpdMinMaxChk(wxCommandEvent& event) {
...
void vsnMPP_Scatter_plotArrows::OnLineWidthTxt(wxCommandEvent& event) {
...
void vsnMPP_Scatter_plotArrows::OnAntiAliasChk(wxCommandEvent& event) {
...
vsnMethod_Scatter_plotArrows::vsnMethod_Scatter_plotArrows(const string& name)
  : vsnMethodObj(name),
...
vsnMethod_Scatter_plotArrows::~vsnMethod_Scatter_plotArrows() {
  if ( m_vectors )
    delete m_vectors;
}
...
void vsnMethod_Scatter_plotArrows::adjustRange() {
...
bool vsnMethod_Scatter_plotArrows::setVecScale(const float vs) {
...
bool vsnMethod_Scatter_plotArrows::setVecHeadMode(const bool vhm) {
...
bool vsnMethod_Scatter_plotArrows::setVecDataIdx(const Vec3<int>& vdidx) {
...
bool vsnMethod_Scatter_plotArrows::setSelectedData(const WhichDataType sd) {
...
bool vsnMethod_Scatter_plotArrows::isValidVecData() const {
  vsnNumericalDataIF* pData = dynamic_cast<vsnNumericalDataIF*>(p_refData);
  if ( ! pData ) return false;
...
bool vsnMethod_Scatter_plotArrows::setUpdateMinMaxMode(const bool mode) {
...
bool vsnMethod_Scatter_plotArrows::setLineWidth(const float lw) {
...
bool vsnMethod_Scatter_plotArrows::update(const bool force) {
  return updateStep(m_requestedStp, force);
}

void vsnMethod_Scatter_plotArrows::reloaded() {
  adjustRange();
  vsnMethodObj::reloaded();
}

vsnMethodPP* vsnMethod_Scatter_plotArrows::getParamPanel(wxPanel* pp) {
  vsnMPP_Scatter_plotArrows* pp_plotArrows
    = new vsnMPP_Scatter_plotArrows(pp, this);
  if ( ! pp_plotArrows ) return NULL;
  return pp_plotArrows;
}


/* vsnTimeSeriesMethodIF methods */

bool vsnMethod_Scatter_plotArrows::updateStep(const int stp, const bool force,
					      const bool cascade)
{
  ...
  if ( m_selectedData == DATA_Veclen ) {
    ...
    for ( i = 0; i < sampleSz; i++ ) {
      ...
      vv[2] = pd[dlen*i + m_vecDataIdx[2]];
      memcpy(nl[i], vv.m_v, sizeof(vector3));
      c = m_lut.getValIdx(pd[dlen*i + m_selectedData -1]);
      cl[i][0] = m_lut.lutEntry[c*4  ];
      cl[i][1] = m_lut.lutEntry[c*4+1];
      cl[i][2] = m_lut.lutEntry[c*4+2];
      cl[i][3] = 1.f;
...
bool vsnMethod_Scatter_plotArrows::parseXML(xmlNodePtr xnp) {
  static xmlChar* xs;
  xmlNodePtr cur;
...
bool vsnMethod_Scatter_plotArrows::outputXML(std::ostream& os, const size_t ts)
{
  std::string idts;
...
bool vsnMethod_Scatter_plotArrows::commandXML(xmlNodePtr xnp) {
  static xmlChar* xs;
  string msgHdr
