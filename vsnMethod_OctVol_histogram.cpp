...
vsnMethod_OctVol_histogram::vsnMethod_OctVol_histogram(const string& name)
  : vsnMethod_histogram(name)
{
  _values = NULL;
}

vsnMethod_OctVol_histogram::~vsnMethod_OctVol_histogram() {
  if ( _values )
    DeAllocate(_values);
}
...
bool vsnMethod_OctVol_histogram::update(const bool force) {
...
bool vsnMethod_OctVol_histogram::updateStep(const int stp,
                                        const bool force, const bool cascade)
{
  vsnData_OctVol* pData = dynamic_cast<vsnData_OctVol*>(p_refData);
  if ( ! pData ) return false;
  const vsnOctTree* pOctree = pData->getOctTree();
  ...
  // counting
  set<vsnOctTree::Node*>::const_iterator it;
  if ( m_selectedData == DATA_Veclen ) {
    Vec3<float> vv;
    for ( it = pOctree->m_pLeafLst.begin();
	  it != pOctree->m_pLeafLst.end(); it ++ ) {
      if ( ! *it ) continue;
      vv.m_v[0] = (*it)->m_pData[m_vecDataIdx.m_v[0]];
      vv.m_v[1] = (*it)->m_pData[m_vecDataIdx.m_v[1]];
      vv.m_v[2] = (*it)->m_pData[m_vecDataIdx.m_v[2]];
      val = vv.Length();
    ...
    for ( it = pOctree->m_pLeafLst.begin();
	  it != pOctree->m_pLeafLst.end(); it ++ ) {
      if ( ! *it ) continue;
      val = (*it)->m_pData[m_selectedData -1];
