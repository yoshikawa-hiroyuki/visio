...
//----------------------------------------------------------------
BEGIN_EVENT_TABLE(vsnMPP_plotVector, wxPanel)
  EVT_TEXT_ENTER(MPP_plotVector_SamplerTxt,
                 vsnMPP_plotVector::OnEnterSamplerTxt)
  EVT_BUTTON(MPP_plotVector_SetSamplerBtn,
             vsnMPP_plotVector::OnSetSamplerBtn)
  EVT_TEXT_ENTER(MPP_plotVector_VecScaleTxt,
                 vsnMPP_plotVector::OnVecScaleTxt)
  EVT_CHECKBOX(MPP_plotVector_VecHeadChk,
               vsnMPP_plotVector::OnVecHeadChk)
  EVT_CHECKLISTBOX(MPP_plotVector_VecDataChkLst,
                   vsnMPP_plotVector::OnVecDataChkLst)
  EVT_COMBOBOX(MPP_plotVector_SelDataLst,
	       vsnMPP_plotVector::OnSelDataLst)
  EVT_CHECKBOX(MPP_plotVector_UpdMinMaxChk,
               vsnMPP_plotVector::OnUpdMinMaxChk)
  EVT_TEXT_ENTER(MPP_plotVector_LineWidthTxt,
                 vsnMPP_plotVector::OnLineWidthTxt)
  EVT_CHECKBOX(MPP_plotVector_AntiAliasChk,
               vsnMPP_plotVector::OnAntiAliasChk)
END_EVENT_TABLE()


/* constructors / destructor */

vsnMPP_plotVector::vsnMPP_plotVector(wxPanel* parent, vsnMethodObj* pm)
  : vsnMethodPP(parent, pm)
{
  assert(parent);
  vsnMethod_plotVector* pMtd = dynamic_cast<vsnMethod_plotVector*>(pm);
  assert(pMtd);

  // create widgets
  m_pSamplerLst = new wxListBox(this, MPP_plotVector_SamplerLst);
  assert(m_pSamplerLst);
  m_pSamplerTxt = new wxTextCtrl(this, MPP_plotVector_SamplerTxt,
                                 wxT(""), wxDefaultPosition, wxDefaultSize,
                                 wxTE_READONLY|wxTE_PROCESS_ENTER);
  assert(m_pSamplerTxt);
  m_pSetSamplerBtn = new wxButton(this, MPP_plotVector_SetSamplerBtn,
                                  wxT("set sampler"));
  assert(m_pSetSamplerBtn);
  m_pVecScaleTxt = new wxTextCtrl(this, MPP_plotVector_VecScaleTxt,
                                  wxT(""), wxDefaultPosition, wxDefaultSize,
                                  wxTE_PROCESS_ENTER);
  assert(m_pVecScaleTxt);
  m_pVecHeadChk = new wxCheckBox(this, MPP_plotVector_VecHeadChk,
                                 wxT("arrow head"));
  assert(m_pVecHeadChk);
  m_pVecDataChkLst = new wxCheckListBox(this, MPP_plotVector_VecDataChkLst);
  assert(m_pVecDataChkLst);
  m_pSelDataLst = new wxComboBox(this, MPP_plotVector_SelDataLst,
				 wxT(""), wxDefaultPosition, wxDefaultSize,
				 0, NULL, wxCB_READONLY);
  assert(m_pSelDataLst);
  m_pUpdMinMaxChk = new wxCheckBox(this, MPP_plotVector_UpdMinMaxChk,
                                   wxT("update lut minmax"));
  assert(m_pUpdMinMaxChk);
  m_pLineWidthTxt = new wxTextCtrl(this, MPP_plotVector_LineWidthTxt,
                                   wxT(""), wxDefaultPosition, wxDefaultSize,
                                   wxTE_PROCESS_ENTER);
  assert(m_pLineWidthTxt);
  m_pAntiAliasChk = new wxCheckBox(this, MPP_plotVector_AntiAliasChk,
                                   wxT("anti-alias line"));
...
vsnMPP_plotVector::~vsnMPP_plotVector() {
}


/* interface */

bool vsnMPP_plotVector::update() {
  ...
  vsnMethod_plotVector* pm
    = dynamic_cast<vsnMethod_plotVector*>(p_method);
  if ( ! pm ) return false;
...
void vsnMPP_plotVector::OnSetSamplerBtn(wxCommandEvent& event) {
  if ( ! m_pSamplerLst || ! m_pSamplerTxt || ! m_pSetSamplerBtn )
    return;
...
void vsnMPP_plotVector::OnEnterSamplerTxt(wxCommandEvent& event) {
...
void vsnMPP_plotVector::OnVecScaleTxt(wxCommandEvent& event) {
...
void vsnMPP_plotVector::OnVecHeadChk(wxCommandEvent& event) {
...
void vsnMPP_plotVector::OnVecDataChkLst(wxCommandEvent& event) {
...
void vsnMPP_plotVector::OnSelDataLst(wxCommandEvent& event) {
...
void vsnMPP_plotVector::OnUpdMinMaxChk(wxCommandEvent& event) {
...
void vsnMPP_plotVector::OnLineWidthTxt(wxCommandEvent& event) {
  ...
  if ( valStr.IsEmpty() ) return;
  float value = (float)atof((const char*)valStr.c_str());
  if ( value <= 0.f ) return;
...
void vsnMPP_plotVector::OnAntiAliasChk(wxCommandEvent& event) {
...
vsnMethod_plotVector::vsnMethod_plotVector(const string& name)
  : vsnMethodObj(name), 
    m_vecScale(1.f), m_vecHead(true), m_lineWidth(1.f),
    m_selectedData(DATA_None), m_updateMinMax(true),
    m_vecDataIdx(0,1,2), m_vectors(NULL), m_useSampler(true)
{
  m_showType = RT_WIRE;
}

vsnMethod_plotVector::~vsnMethod_plotVector() {
  if ( m_vectors )
    delete m_vectors;
}
...
void vsnMethod_plotVector::adjustRange() {
...
bool vsnMethod_plotVector::setVecScale(const float vs) {
...
bool vsnMethod_plotVector::setVecHeadMode(const bool vhm) {
...
bool vsnMethod_plotVector::setVecDataIdx(const Vec3<int>& vdidx) {
...
bool vsnMethod_plotVector::setSelectedData(const WhichDataType sd) {
...
bool vsnMethod_plotVector::isValidVecData() const {
  vsnNumericalDataIF* pData = dynamic_cast<vsnNumericalDataIF*>(p_refData);
  if ( ! pData ) return false;
...
bool vsnMethod_plotVector::setUpdateMinMaxMode(const bool mode) {
...
bool vsnMethod_plotVector::setLineWidth(const float lw) {
...
bool vsnMethod_plotVector::update(const bool force) {
  return updateStep(m_requestedStp, force);
}

void vsnMethod_plotVector::reloaded() {
  adjustRange();
  vsnMethodObj::reloaded();
}

vsnMethodPP* vsnMethod_plotVector::getParamPanel(wxPanel* pp) {
  vsnMPP_plotVector* pp_plotVector
    = new vsnMPP_plotVector(pp, this);
  if ( ! pp_plotVector ) return NULL;
  return pp_plotVector;
}


/* from vsnRef_sampler */

void vsnMethod_plotVector::noticeUpdate() {
  update();
}
...
bool vsnMethod_plotVector::parseXML(xmlNodePtr xnp) {
  static xmlChar* xs;
  xmlNodePtr cur;
...
bool vsnMethod_plotVector::outputXML(std::ostream& os, const size_t ts)
{
  std::string idts;
...
bool vsnMethod_plotVector::commandXML(xmlNodePtr xnp) {
  static xmlChar* xs;
  string msgHdr
