...
BEGIN_EVENT_TABLE(vsnMPP_OctVol_shaderVolren, wxPanel)
  EVT_COMBOBOX(MPP_OctVol_shaderVolren_SelDataLst,
               vsnMPP_OctVol_shaderVolren::OnSelDataLst)
  EVT_CHECKLISTBOX(MPP_OctVol_shaderVolren_VecDataChkLst,
                   vsnMPP_OctVol_shaderVolren::OnVecDataChkLst)
  EVT_CHECKBOX(MPP_OctVol_shaderVolren_UpdMinMaxChk,
               vsnMPP_OctVol_shaderVolren::OnUpdMinMaxChk)
  EVT_TEXT_ENTER(MPP_OctVol_shaderVolren_SliceNumTxt,
                 vsnMPP_OctVol_shaderVolren::OnSliceNumTxt)
  EVT_CHECKBOX(MPP_OctVol_shaderVolren_InterpolateChk,
               vsnMPP_OctVol_shaderVolren::OnInterpolateChk)
  EVT_BUTTON(MPP_OctVol_shaderVolren_GradMapBtn,
             vsnMPP_OctVol_shaderVolren::OnGradMapBtn)
  EVT_BUTTON(MPP_OctVol_shaderVolren_RegionResetBtn,
             vsnMPP_OctVol_shaderVolren::OnRegionResetBtn)
  EVT_TEXT_ENTER(MPP_OctVol_shaderVolren_RegionX1Txt,
                 vsnMPP_OctVol_shaderVolren::OnRegionTxt)
  EVT_TEXT_ENTER(MPP_OctVol_shaderVolren_RegionX2Txt,
                 vsnMPP_OctVol_shaderVolren::OnRegionTxt)
  EVT_TEXT_ENTER(MPP_OctVol_shaderVolren_RegionY1Txt,
                 vsnMPP_OctVol_shaderVolren::OnRegionTxt)
  EVT_TEXT_ENTER(MPP_OctVol_shaderVolren_RegionY2Txt,
                 vsnMPP_OctVol_shaderVolren::OnRegionTxt)
  EVT_TEXT_ENTER(MPP_OctVol_shaderVolren_RegionZ1Txt,
                 vsnMPP_OctVol_shaderVolren::OnRegionTxt)
  EVT_TEXT_ENTER(MPP_OctVol_shaderVolren_RegionZ2Txt,
                 vsnMPP_OctVol_shaderVolren::OnRegionTxt)
END_EVENT_TABLE()
...
vsnMPP_OctVol_shaderVolren::vsnMPP_OctVol_shaderVolren(wxPanel* parent,
						       vsnMethodObj* pm)
  : vsnMethodPP(parent, pm)
{
  assert(parent);
  assert(dynamic_cast<vsnMethod_OctVol_shaderVolren*>(pm));
  ...
  m_pSelDataLst = new wxComboBox(this, MPP_OctVol_shaderVolren_SelDataLst,
  ...
  // update minmax mode
  m_pUpdMinMaxChk = new wxCheckBox(this, MPP_OctVol_shaderVolren_UpdMinMaxChk,
                                   wxT("update minmax"));
  ...
  sizerH->Add(new wxStaticText(this, -1, wxT("#of slices")),
              0, wxALIGN_LEFT|wxALL, 3);
  m_pSliceNumTxt = new wxTextCtrl(this, MPP_OctVol_shaderVolren_SliceNumTxt,
				  wxT(""), wxDefaultPosition, wxDefaultSize,
                                  wxTE_PROCESS_ENTER);
  sizerH->Add(m_pSliceNumTxt, 0, wxEXPAND|wxALL, 3);

  // interpolate mode
  m_pInterpolateChk
    = new wxCheckBox(this, MPP_OctVol_shaderVolren_InterpolateChk,
		     wxT("interpolate"));
  ...
  sizerH->Add(new wxStaticText(this, -1, wxT("crop region")),
              0, wxALIGN_LEFT|wxALL, 3);
  m_pRegionResetBtn = new wxButton(this, MPP_OctVol_shaderVolren_RegionResetBtn,
                               wxT("reset region"),
  ...
  m_pRegionX1Txt = new wxTextCtrl(this, MPP_OctVol_shaderVolren_RegionX1Txt,
  ...
  m_pRegionX2Txt = new wxTextCtrl(this, MPP_OctVol_shaderVolren_RegionX2Txt,
  ...
  m_pRegionY1Txt = new wxTextCtrl(this, MPP_OctVol_shaderVolren_RegionY1Txt,
                                  wxT("0"), wxDefaultPosition, wxSize(60,-1),
                                  wxTE_PROCESS_ENTER);
  sizerH->Add(m_pRegionY1Txt, 1, wxEXPAND|wxALL, 3);
  sizerH->Add(new wxStaticText(this, -1, wxT("Y2")), 0, wxALIGN_LEFT|wxALL, 3);
  m_pRegionY2Txt = new wxTextCtrl(this, MPP_OctVol_shaderVolren_RegionY2Txt,
                                  wxT("1"), wxDefaultPosition, wxSize(60,-1),
                                  wxTE_PROCESS_ENTER);
  ...
  m_pRegionZ1Txt = new wxTextCtrl(this, MPP_OctVol_shaderVolren_RegionZ1Txt,
                                  wxT("0"), wxDefaultPosition, wxSize(60,-1),
                                  wxTE_PROCESS_ENTER);
  sizerH->Add(m_pRegionZ1Txt, 1, wxEXPAND|wxALL, 3);
  sizerH->Add(new wxStaticText(this, -1, wxT("Z2")), 0, wxALIGN_LEFT|wxALL, 3);
  m_pRegionZ2Txt = new wxTextCtrl(this, MPP_OctVol_shaderVolren_RegionZ2Txt,
                                  wxT("1"), wxDefaultPosition, wxSize(60,-1),
                                  wxTE_PROCESS_ENTER);
  ...
  // grad-map button
  topsizer->Add(5, 5);
  m_pGradMapBtn = new wxButton(this, MPP_OctVol_shaderVolren_GradMapBtn,
...
vsnMPP_OctVol_shaderVolren::~vsnMPP_OctVol_shaderVolren() {
}


/* interface */

bool vsnMPP_OctVol_shaderVolren::update() {
  if ( ! m_pSelDataLst || ! m_pVecDataChkLst ||
       ! m_pUpdMinMaxChk || ! m_pSliceNumTxt || ! m_pInterpolateChk ||
       ! m_pRegionX1Txt || ! m_pRegionY1Txt || ! m_pRegionZ1Txt ||
       ! m_pRegionX2Txt || ! m_pRegionY2Txt || ! m_pRegionZ2Txt )
    return false;

  vsnMethod_OctVol_shaderVolren* pm
    = dynamic_cast<vsnMethod_OctVol_shaderVolren*>(p_method);
  if ( ! pm ) return false;
...
void vsnMPP_OctVol_shaderVolren::OnSelDataLst(wxCommandEvent& event) {
...
void vsnMPP_OctVol_shaderVolren::OnVecDataChkLst(wxCommandEvent& event) {
...
void vsnMPP_OctVol_shaderVolren::OnUpdMinMaxChk(wxCommandEvent& event) {
...
void vsnMPP_OctVol_shaderVolren::OnSliceNumTxt(wxCommandEvent& event) {
...
void vsnMPP_OctVol_shaderVolren::OnInterpolateChk(wxCommandEvent& event) {
...
void vsnMPP_OctVol_shaderVolren::OnGradMapBtn(wxCommandEvent& event) {
  vsnMethod_OctVol_shaderVolren* pm
    = dynamic_cast<vsnMethod_OctVol_shaderVolren*>(p_method);
  if ( ! pm ) return;
...
void vsnMPP_OctVol_shaderVolren::OnRegionResetBtn(wxCommandEvent& event) {
  vsnMethod_OctVol_shaderVolren* pm
    = dynamic_cast<vsnMethod_OctVol_shaderVolren*>(p_method);
  if ( ! pm ) return;
  CES::Vec3<float> region[2];
...
void vsnMPP_OctVol_shaderVolren::OnRegionTxt(wxCommandEvent& event) {
...
vsnMethod_OctVol_shaderVolren::
vsnMethod_OctVol_shaderVolren(const std::string& name)
  : vsnMethodObj(name), m_pRender(NULL),
...
vsnMethod_OctVol_shaderVolren::~vsnMethod_OctVol_shaderVolren() {
  if ( m_pRender ) {
    if ( p_refData ) {
...
void vsnMethod_OctVol_shaderVolren::adjustRange() {
...
bool vsnMethod_OctVol_shaderVolren::setSelectedData(const WhichDataType sd) {
...
bool
vsnMethod_OctVol_shaderVolren::setVecDataIdx(const CES::Vec3<int>& vdidx) {
...
bool vsnMethod_OctVol_shaderVolren::isValidVecData() const {
  vsnData_OctVol* pData = dynamic_cast<vsnData_OctVol*>(p_refData);
  if ( ! pData ) return false;
...
bool vsnMethod_OctVol_shaderVolren::setUpdateMinMaxMode(const bool mode) {
...
bool vsnMethod_OctVol_shaderVolren::setInterpolateMode(const bool mode) {
...
size_t vsnMethod_OctVol_shaderVolren::getNumSlices() const {
  if ( ! m_pRender ) return 0;
  return m_pRender->GetSliceNum();
}

bool vsnMethod_OctVol_shaderVolren::setNumSlices(const size_t nsl) {
...
bool vsnMethod_OctVol_shaderVolren::setRegion(const CES::Vec3<float>* preg) {
...
bool vsnMethod_OctVol_shaderVolren::getRegion(CES::Vec3<float>* preg) const {
  if ( ! preg ) return false;
  if ( ! m_pRender ) return false;
...
bool vsnMethod_OctVol_shaderVolren::updateStep(const int stp, const bool force,
					       const bool cascade)
{
  vsnData_OctVol* pData = dynamic_cast<vsnData_OctVol*>(p_refData);
  if ( ! pData ) return false;
  ...
  if ( ! m_pRender->SetData(const_cast<vsnOctTree*>(pData->getOctTree()),
			    m_selectedData, m_vecDataIdx,
			    dr, m_interpolate) ) {
    ErrMsg(MsgERR, getMethodType() + string("[") + getName()
...
void vsnMethod_OctVol_shaderVolren::ext_setLut(const vsnLut& lut) {
  if ( ! m_pRender ) return;
  if ( m_pRender->SetDerivLut(lut) )
    chkNotice();
}

vsnLut vsnMethod_OctVol_shaderVolren::ext_getLut() const {
  vsnLut lut;
...
bool vsnMethod_OctVol_shaderVolren::update(const bool force) {
  return updateStep(m_requestedStp, force);
}

void vsnMethod_OctVol_shaderVolren::reloaded() {
  adjustRange();
  vsnMethodObj::reloaded();
}

vsnMethodPP* vsnMethod_OctVol_shaderVolren::getParamPanel(wxPanel* pp) {
  vsnMPP_OctVol_shaderVolren* pp_shaderVolren
    = new vsnMPP_OctVol_shaderVolren(pp, this);
  if ( ! pp_shaderVolren ) return NULL;
  return pp_shaderVolren;
}

void vsnMethod_OctVol_shaderVolren::setBaseColor(const vector4 cv) {
...
void vsnMethod_OctVol_shaderVolren::setRefData(vsnDataObj* prd) {
...
void vsnMethod_OctVol_shaderVolren::setLighting(const bool mode) {
  if ( m_pRender )
...
bool vsnMethod_OctVol_shaderVolren::parseXML(xmlNodePtr xnp) {
  static xmlChar* xs;
  xmlNodePtr cur;
...
bool
vsnMethod_OctVol_shaderVolren::outputXML(std::ostream& os, const size_t ts) {
  size_t i;
  std::string idts;
...
bool vsnMethod_OctVol_shaderVolren::commandXML(xmlNodePtr xnp) {
  static xmlChar* xs;
  string msgHdr
