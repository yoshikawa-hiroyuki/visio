...
BEGIN_EVENT_TABLE(vsnMPP_OctVol_isosurf, wxPanel)
  EVT_COMBOBOX(MPP_OctVol_isosurf_SelDataLst,
               vsnMPP_OctVol_isosurf::OnSelDataLst)
  EVT_CHECKLISTBOX(MPP_OctVol_isosurf_VecDataChkLst,
                   vsnMPP_OctVol_isosurf::OnVecDataChkLst)
  EVT_TEXT_ENTER(MPP_OctVol_isosurf_ValueTxt,
                 vsnMPP_OctVol_isosurf::OnValueTxt)
  EVT_CHECKBOX(MPP_OctVol_isosurf_UseCMapChk,
               vsnMPP_OctVol_isosurf::OnUseCMapChk)
  EVT_CHECKBOX(MPP_OctVol_isosurf_UseCMapAlphaChk,
               vsnMPP_OctVol_isosurf::OnUseCMapAlphaChk)
  EVT_CHECKBOX(MPP_OctVol_isosurf_UpdMinMaxChk,
               vsnMPP_OctVol_isosurf::OnUpdMinMaxChk)
  EVT_TEXT_ENTER(MPP_OctVol_isosurf_LevelTxt,
                 vsnMPP_OctVol_isosurf::OnLevelTxt)
  EVT_CHECKBOX(MPP_OctVol_isosurf_InterpolateChk,
               vsnMPP_OctVol_isosurf::OnInterpolateChk)
  EVT_RADIOBOX(MPP_OctVol_isosurf_ShowFaceRadio,
	       vsnMPP_OctVol_isosurf::OnShowFaceRadio)
  EVT_BUTTON(MPP_OctVol_isosurf_ExportBtn,
	     vsnMPP_OctVol_isosurf::OnExportBtn)
  EVT_CHECKBOX(MPP_OctVol_isosurf_AutoExportChk,
	       vsnMPP_OctVol_isosurf::OnAutoExportChk)
END_EVENT_TABLE()


/* constructors / destructor */

vsnMPP_OctVol_isosurf::vsnMPP_OctVol_isosurf(wxPanel* parent, vsnMethodObj* pm)
  : vsnMethodPP(parent, pm)
{
  assert(parent);
  assert(dynamic_cast<vsnMethod_OctVol_isosurf*>(pm));
  ...
  m_pValueTxt = new wxTextCtrl(this, MPP_OctVol_isosurf_ValueTxt, wxT(""),
  ...
  sizerH = new wxBoxSizer(wxHORIZONTAL);
  topsizer->Add(sizerH, 0, wxALL, 0);
  sizerH->Add(new wxStaticText(this, -1, wxT("oct level")),
              0, wxALIGN_LEFT|wxALL, 3);
  m_pLevelTxt = new wxTextCtrl(this, MPP_OctVol_isosurf_LevelTxt, wxT(""),
                               wxDefaultPosition, wxDefaultSize,
                               wxTE_PROCESS_ENTER);
  sizerH->Add(m_pLevelTxt, 0, wxEXPAND|wxALL, 3);


  // data selector
  topsizer->Add(new wxStaticText(this, -1, wxT("select scalar data")),
                0, wxALIGN_LEFT|wxALL, 3);
  m_pSelDataLst = new wxComboBox(this, MPP_OctVol_isosurf_SelDataLst,
  ...
  // use CMap / CMap alpha
  m_pUseCMapChk = new wxCheckBox(this, MPP_OctVol_isosurf_UseCMapChk,
                                 wxT("use cmap"));
  m_pUseCMapChk->SetValue(TRUE);
  topsizer->Add(m_pUseCMapChk, 0, wxALL, 3);
  m_pUseCMapAlphaChk = new wxCheckBox(this, MPP_OctVol_isosurf_UseCMapAlphaChk,
                                      wxT("use cmap alpha"));
  topsizer->Add(m_pUseCMapAlphaChk, 0, wxALL, 3);

  // update minmax mode
  m_pUpdMinMaxChk = new wxCheckBox(this, MPP_OctVol_isosurf_UpdMinMaxChk,
                                   wxT("update minmax"));
  m_pUpdMinMaxChk->SetValue(TRUE);
  topsizer->Add(m_pUpdMinMaxChk, 0, wxALL, 3);

  // interpolate mode
  m_pInterpolateChk = new wxCheckBox(this, MPP_OctVol_isosurf_InterpolateChk,
                                     wxT("interpolate"));
  ...
  m_pShowFaceRadio = new wxRadioBox(this, MPP_OctVol_isosurf_ShowFaceRadio,
				    wxT("show face"),
				    wxDefaultPosition, wxDefaultSize,
  ...
  m_pExportBtn = new wxButton(this, MPP_OctVol_isosurf_ExportBtn,
			      wxT("export"));
  sizerH->Add(m_pExportBtn, 0, wxALL, 3);
  m_pAutoExportChk = new wxCheckBox(this, MPP_OctVol_isosurf_AutoExportChk,
				    wxT("auto export"));
...
vsnMPP_OctVol_isosurf::~vsnMPP_OctVol_isosurf() {
}


/* interface */

bool vsnMPP_OctVol_isosurf::update() {
  if ( ! m_pSelDataLst || ! m_pVecDataChkLst || ! m_pValueTxt ||
       ! m_pUseCMapChk || ! m_pUseCMapAlphaChk || ! m_pUpdMinMaxChk ||
       ! m_pLevelTxt || ! m_pInterpolateChk || ! m_pShowFaceRadio)
    return false;

  vsnMethod_OctVol_isosurf* pm
    = dynamic_cast<vsnMethod_OctVol_isosurf*>(p_method);
  if ( ! pm ) return false;
  ...
  // level
  size_t lvl = pm->getLevel();
  sprintf(txt, "%zd", lvl); m_pLevelTxt->SetValue(vsnApp::ConvSysToWx(txt));
...
void vsnMPP_OctVol_isosurf::OnSelDataLst(wxCommandEvent& event) {
...
void vsnMPP_OctVol_isosurf::OnVecDataChkLst(wxCommandEvent& event) {
...
void vsnMPP_OctVol_isosurf::OnValueTxt(wxCommandEvent& event) {
...
void vsnMPP_OctVol_isosurf::OnUseCMapChk(wxCommandEvent& event) {
...
void vsnMPP_OctVol_isosurf::OnUseCMapAlphaChk(wxCommandEvent& event) {
...
void vsnMPP_OctVol_isosurf::OnUpdMinMaxChk(wxCommandEvent& event) {
...
void vsnMPP_OctVol_isosurf::OnLevelTxt(wxCommandEvent& event) {
  if ( ! m_pLevelTxt ) return;

  vsnMethod_OctVol_isosurf* pm
    = dynamic_cast<vsnMethod_OctVol_isosurf*>(p_method);
  if ( ! pm ) return;

  wxString valStr = m_pLevelTxt->GetValue();
  if ( valStr.IsEmpty() ) {
    char txt[64]; sprintf(txt, "%zd", pm->getLevel());
    m_pLevelTxt->SetValue(vsnApp::ConvSysToWx(txt));
    return;
  }
  int lvl = atoi(vsnApp::ConvWxToSys(valStr).c_str());
  if ( lvl < 0 ) {
    char txt[64]; sprintf(txt, "%zd", pm->getLevel());
    m_pLevelTxt->SetValue(vsnApp::ConvSysToWx(txt));
    return;
...
void vsnMPP_OctVol_isosurf::OnInterpolateChk(wxCommandEvent& event) {
...
void vsnMPP_OctVol_isosurf::OnShowFaceRadio(wxCommandEvent& event) {
  vsnMethod_OctVol_isosurf* pm
    = dynamic_cast<vsnMethod_OctVol_isosurf*>(p_method);
  if ( ! pm ) return;
...
void vsnMPP_OctVol_isosurf::OnExportBtn(wxCommandEvent& event) {
  static int fltIdx = 0;
...
void vsnMPP_OctVol_isosurf::OnAutoExportChk(wxCommandEvent& event) {
...
vsnMethod_OctVol_isosurf::
vsnMethod_OctVol_isosurf(const std::string& name)
  : vsnMethodObj(name), m_isoTrias(NULL), m_pd(NULL),
    m_selectedData(DATA_None), m_vecDataIdx(0,1,2), m_isoValue(0.f),
    m_useCMap(true), m_useCMapAlpha(false), m_updateMinMax(true), m_level(0),
    m_rsmpl_updStamp(-1), m_rsmpl_updLevel(0), m_interpolate(false),
    m_faceMode(PF_BOTH), m_autoExport(false)
{
}

vsnMethod_OctVol_isosurf::~vsnMethod_OctVol_isosurf() {
  if ( m_isoTrias )
    delete m_isoTrias;
  if ( m_pd )
    DeAllocate(m_pd);
...
void vsnMethod_OctVol_isosurf::adjustRange(const bool updval) {
...
void vsnMethod_OctVol_isosurf::updateColor() {
...
bool vsnMethod_OctVol_isosurf::setSelectedData(const WhichDataType sd) {
...
bool vsnMethod_OctVol_isosurf::setVecDataIdx(const CES::Vec3<int>& vdidx) {
...
bool vsnMethod_OctVol_isosurf::isValidVecData() const {
  vsnData_OctVol* pData = dynamic_cast<vsnData_OctVol*>(p_refData);
  if ( ! pData ) return false;
...
bool vsnMethod_OctVol_isosurf::setIsoValue(const float val) {
...
bool vsnMethod_OctVol_isosurf::setUseCMap(const bool ucm) {
...
bool vsnMethod_OctVol_isosurf::setUseCMapAlpha(const bool ucma) {
...
bool vsnMethod_OctVol_isosurf::setUpdateMinMaxMode(const bool mode) {
...
bool vsnMethod_OctVol_isosurf::setLevel(const size_t lvl) {
  if ( m_level == lvl ) return true;
  m_level = lvl;
...
bool vsnMethod_OctVol_isosurf::setInterpolateMode(const bool mode) {
...
bool vsnMethod_OctVol_isosurf::setShowFaceMode(const PolyFaceType fm) {
...
bool vsnMethod_OctVol_isosurf::exportFaces(const std::string& path) {
...
bool vsnMethod_OctVol_isosurf::setAutoExport(const bool aem) {
...
bool vsnMethod_OctVol_isosurf::updateStep(const int stp,
					  const bool force, const bool cascade)
{
  ...
  if ( level > pData->getMaxLevel() ) level = pData->getMaxLevel();
  if ( m_rsmpl_updStamp != pData->getOctTreeUpdatedStamp() ||
       m_rsmpl_updLevel != level ) {
    vsnOctTree* pot = const_cast<vsnOctTree*>(pData->getOctTree());
    if ( ! m_rsmpl.Setup(pot, false, true, m_interpolate, level) ) {
      ErrMsg(MsgERR, getMethodType() + string("[") + getName()
  ...
  vsnRsmplOctVol::RsmplBlk* pRsmplBlk = rsmplBlkLst[0];
  if ( ! pRsmplBlk ) return false;
  const float* dptr = pRsmplBlk->m_pData;
  if ( ! dptr ) return false;

  Vec3<size_t> dims = pRsmplBlk->m_dims;
  ...
  // generate isosurf
  if ( ! vsnIsosurf::IsoSurf(dims.m_v, NULL,
  ...
  // set geometry
  m_isoTrias->identity();
  m_isoTrias->trans(pRsmplBlk->m_bbox[0].m_v);
  Vec3<float> blkBbLen(dims[0]-1.f, dims[1]-1.f, dims[2]-1.f);
  Vec3<float> scaleVec = (pRsmplBlk->m_bbox[1] - pRsmplBlk->m_bbox[0]);
  scaleVec.m_v[0] /= blkBbLen.m_v[0];
  scaleVec.m_v[1] /= blkBbLen.m_v[1];
  scaleVec.m_v[2] /= blkBbLen.m_v[2];
  m_isoTrias->scale(scaleVec.m_v);
...
bool vsnMethod_OctVol_isosurf::update(const bool force) {
  return updateStep(m_requestedStp, force);
}

void vsnMethod_OctVol_isosurf::reloaded() {
  adjustRange(false);
  vsnMethodObj::reloaded();
}

vsnMethodPP* vsnMethod_OctVol_isosurf::getParamPanel(wxPanel* pp) {
  vsnMPP_OctVol_isosurf* pp_isosurf
    = new vsnMPP_OctVol_isosurf(pp, this);
  if ( ! pp_isosurf ) return NULL;
  return pp_isosurf;
}

void vsnMethod_OctVol_isosurf::setBaseColor(const vector4 cv) {
...
bool vsnMethod_OctVol_isosurf::parseXML(xmlNodePtr xnp) {
  static xmlChar* xs;
  xmlNodePtr cur;
  ...
  while ( cur ) {
    ...
    if ( !strcmp((const char*)cur->name, "param") ) {
      ...
      else if ( xsN == string("oct_level") ) {
        int val = atoi(xsV.c_str());
        if ( val < 0 || ! setLevel(val) ) {
          ErrMsg(MsgERR, msgHdr + string("invalid value in param oct_level"));
          goto _NEXT_XML_NODE;
        }
      } // end of "oct_level"
...
bool vsnMethod_OctVol_isosurf::outputXML(std::ostream& os, const size_t ts) {
  std::string idts;
  ...
  // oct_level
  os << idts_2 << "<param name=\"oct_level\" value=\""
     << m_level << "\" />" << endl;
...
bool vsnMethod_OctVol_isosurf::commandXML(xmlNodePtr xnp) {
  static xmlChar* xs;
  string msgHdr
  ...
  else if ( nameStr == "set_oct_level" ) {
    int val = atoi(valueStr.c_str());
    if ( val < 0 || ! setLevel(val) ) {
      ErrMsg(MsgERR, msgHdr +
             string("command set_oct_level: set failed: ") + valueStr);
      return false;
    }
  } // end of "set_oct_level"
...
void vsnMethod_OctVol_isosurf::generateBbox() {
  vsnData_OctVol* pData = dynamic_cast<vsnData_OctVol*>(p_refData);
  if ( ! pData ) return;
